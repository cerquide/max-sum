...
//	val x = new DenseIntIndex(2)
}

class MaxSumCell(id: Any, initialState: Int) extends DataGraphVertex(id, initialState) {
  type Signal = Int
...
object MaxSum extends App {
  ...
  for (column <- 0 to columns; row <- 0 to rows) {
    graph.addVertex(new MaxSumCell((column, row), (math.random * 2.0).floor.toInt))
  }
